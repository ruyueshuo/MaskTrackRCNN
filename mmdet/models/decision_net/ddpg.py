...
class ActorNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        feat_history = torch.from_numpy(inputs[3]).cuda().float().reshape((1, -1))
...
class CriticNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.fc2 = nn.Linear(2048, 512)
        self.fc3 = nn.Linear(512, 128)
        self.fc4 = nn.Linear(128+7, num_outputs)
    ...
    def forward(self, state, action):
        """mu, sigma_sq?"""
        feat_diff = state[0].cuda()
        feat_self = state[1].cuda()
        feat_FAR = torch.tensor(state[2]).cuda().float().reshape((1, -1))
        feat_history = torch.from_numpy(state[3]).cuda().float().reshape((1, -1))
        x = torch.cat((feat_diff, feat_self), 1)
        y = torch.cat((feat_FAR, feat_history), 1)
        x = self.conv(x)
        x = self.bn(x)
